...
async def send_calculator_lead_to_telegram(lead: PriceCalculatorLead) -> bool:
    """
    Send the price calculator lead as a formatted Telegram message.
    Returns True on success, False if Telegram is mis-configured or the call fails.
    """
    token = settings.TELEGRAM_BOT_TOKEN_NN
    chat_id = settings.TELEGRAM_CHAT_ID_NN
    ...
    payload = {
        "chat_id": chat_id,
        "text": TelegramMessageBuilder.build_calculator(lead),
        "parse_mode": "HTML",
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            response = await client.post(url, json=payload)
            response.raise_for_status()
            logger.info("Telegram notification sent for calculator lead: %s", lead.contact)
            return True
    except httpx.HTTPStatusError as exc:
        logger.error(
            "Telegram API returned error %s: %s",
